...
# List of Ensembl IDs for GO:0007626
...
# Connect to A. calliptera dataset in BioMart
ensembl <- useEnsembl(biomart = "ensembl", dataset = "acalliptera_gene_ensembl", mirror = "useast")

# Get gene names and descriptions
gene_annotations <- getBM(attributes = c("ensembl_gene_id", "external_gene_name", "description"),
                          filters = "ensembl_gene_id", values = loco_genes, mart = ensembl)

# Save results
write.table(gene_annotations, "Tables/Locomotory_gene_names.tsv", sep = "\t", row.names = FALSE)