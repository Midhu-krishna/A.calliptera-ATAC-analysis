...
#3dpf
plot_3dpf <- ggplot(sig_data_3dpf, aes(x = fold, y = annotation, size = overlap_nsegments, color = log_qvalue)) +
  geom_point() +  # Draw circles
  scale_size(name = "Peak Count", range = c(2, 10)) +  # Size for peak count
  scale_color_gradient(low = "blue", high = "red", name = "-log10(FDR)") +  # Color for significance
...
#7dpf
plot_7dpf <- ggplot(sig_data_7dpf, aes(x = fold, y = annotation, size = overlap_nsegments, color = log_qvalue)) +
  geom_point() +  # Draw circles
  scale_size(name = "Peak Count", range = c(2, 10)) +  # Size for peak count
  scale_color_gradient(low = "blue", high = "red", name = "-log10(FDR)") +  # Color for significance
  theme_minimal() +  # Simple theme
...
#12dpf
plot_12dpf <- ggplot(sig_data_12dpf, aes(x = fold, y = annotation, size = overlap_nsegments, color = log_qvalue)) +
  geom_point() +  # Draw circles
  scale_size(name = "Peak Count", range = c(2, 10)) +  # Size for peak count
  scale_color_gradient(low = "blue", high = "red", name = "-log10(FDR)") +  # Color for significance
  theme_minimal() +  # Simple theme
...
all_data <- rbind(data_3dpf, data_7dpf, data_12dpf)

# Keep only significant results (qvalue < 0.05)
sig_data <- all_data[all_data$qvalue < 0.05, ]

# Calculate -log10(qvalue) for color
sig_data$log_qvalue <- -log10(sig_data$qvalue)

# Make the dotplot
combined_plot <- ggplot(sig_data, aes(x = fold, y = annotation, size = overlap_nsegments, color = log_qvalue)) +
  geom_point() +  # Draw circles
  facet_grid(. ~ stage) +  # Separate by stage
  scale_size(name = "Peak Count", range = c(2, 10)) +  # Size for peak count
  scale_color_gradient(low = "blue", high = "red", name = "-log10(FDR)") +  # Color for significance
  theme_minimal() +  # Simple theme
  theme(
    axis.text = element_text(size = 10),  # Text size
    axis.title = element_text(size = 12),  # Title size
    strip.text = element_text(size = 12, face = "bold")  # Stage labels
  ) +
  labs(
    x = "Fold Enrichment",  # X-axis label
    y = "Genomic Region",  # Y-axis label
    title = "Peak Enrichment in A. calliptera"
  )
show(combined_plot)
...
summary <- data.frame(
  ...
  Annotation = sig_data$annotation,
  Fold = sig_data$fold,
  Peak_Count = sig_data$overlap_nsegments,
  Qvalue = sig_data$qvalue
)
write.table(summary, "simple_summary.txt", sep = "\t", row.names = FALSE)